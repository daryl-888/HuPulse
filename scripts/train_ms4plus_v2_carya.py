"""
Standalone MS4 training with architectural improvements (v2).

Same training protocol as the paper (Adam lr=2e-5, 100 epochs, MSE, 10-fold CV)
and same data pipeline as train_ms4plus_carya.py — only the MODEL ARCHITECTURE
is different.

New options (all architectural — no training protocol changes):
  --per-layer-film   Apply FiLM at every S4 block (like ViT1D_FiLM does at
                     every transformer layer). Default: FiLM once after all blocks.
  --d-state N        Override S4 state size (default: 64).
  --d-model N        Override S4 model dimension (default: 256; paper MS4 uses 512).

Usage:
    python scripts/train_ms4plus_v2_carya.py --bp SBP --variant film --per-layer-film
    python scripts/train_ms4plus_v2_carya.py --bp DBP --variant film --d-state 128 --d-model 512

Output directories use _v2 suffix to avoid colliding with v1 results.
"""
import os
import sys
import csv
...
# =============================================================================
# Building blocks — matches deploy/MS4.py and train_ms4plus_carya.py
# =============================================================================
...
# =============================================================================
# V2: Per-layer FiLM variant
# =============================================================================

class S4Block_FiLM(nn.Module):
    """S4Block with integrated per-layer FiLM conditioning."""
    def __init__(self, d_model, d_state=64, l_max=2048, dropout=0.2, bidirectional=True,
                 static_hidden=32):
        super().__init__()
        self.block = S4Block(d_model, d_state=d_state, l_max=l_max,
                             dropout=dropout, bidirectional=bidirectional)
        self.film = _FiLM(static_hidden, d_model)

    def forward(self, x, s, rate=1.0):  # x: (B, C, L), s: (B, static_hidden)
        x = self.block(x, rate=rate)
        x_t = x.transpose(-1, -2)       # (B, L, C)
        x_t = self.film(x_t, s)          # apply FiLM in (B, L, C) space
        return x_t.transpose(-1, -2)     # back to (B, C, L)


# =============================================================================
# V2: MS4 + Per-Layer FiLM
# =============================================================================

class MS4_FiLM_PerLayer(nn.Module):
    """
    Conv stem → 4× S4Block_FiLM → AttnPool → head.

    Each S4 block has its OWN FiLM module, so demographics influence signal
    processing at every stage — same pattern as ViT1D_FiLM in the paper.
    """
    def __init__(self, num_static_features=3, num_BP=1,
                 ...
                 s4_l_max=2048, s4_d_state=64, dropout=0.2, bidirectional=True):
        super().__init__()
        D = s4_d_model
        ...
        self.blocks = nn.ModuleList([
            S4Block_FiLM(D, d_state=s4_d_state, l_max=s4_l_max, dropout=dropout,
                         bidirectional=bidirectional, static_hidden=static_hidden)
...
# =============================================================================
# V1 models (copied from train_ms4plus_carya.py — unchanged)
# =============================================================================
...
class MS4_FiLMOnly(nn.Module):
    ...
    def forward(self, ppg, static_feat):
        feat = self.s4model(ppg)
        d = self.demo_mlp(static_feat)
...
# =============================================================================
# Variant registry — V2 models
# =============================================================================

VARIANTS = {
    "film":           MS4_FiLM,          # V1: FiLM once after all blocks
    "gate":           MS4_Gate,          # V1: Gate once after all blocks
    "filmgate":       MS4_FiLMGate,      # V1: FiLM+Gate once after all blocks
    "film_baseline":  MS4_FiLMOnly,      # V1: Minimal ablation
    "film_perlayer":  MS4_FiLM_PerLayer, # V2: Per-layer FiLM (NEW)
}
...
def main():
    ...
    parser.add_argument("--variant", choices=list(VARIANTS),  default="film_perlayer",
                        help="Fusion variant: film | gate | filmgate | film_baseline | film_perlayer")
    ...
    parser.add_argument("--d-state", type=int, default=None,
                        help="S4 state size (default: 64)")
    parser.add_argument("--d-model", type=int, default=None,
                        help="S4 model dimension (default: 256)")
    parser.set_defaults(bidir=True)
    args = parser.parse_args()

    BP, variant, bidir = args.bp, args.variant, args.bidir
    bidir_tag = "bidir" if bidir else "uni"
    model_tag  = f"d{args.d_model}" if args.d_model else ""
    state_tag  = f"s{args.d_state}" if args.d_state else ""
    extra = "_".join(filter(None, [model_tag, state_tag]))
    tag_parts = [f"ms4_{variant}_{bidir_tag}_v2"]
    if extra:
        tag_parts.append(extra)
    OUT_DIR = "_".join(tag_parts) + f"_cv_{BP.lower()}"
    os.makedirs(OUT_DIR, exist_ok=True)

    Seed(SEED)
    print(f"BP={BP}  variant={variant}  bidirectional={bidir}")
    print(f"d_model={args.d_model or 'default'}  d_state={args.d_state or 'default'}")
    print(f"out={OUT_DIR}")
    ...
    for fold_id, (train_idx, _) in enumerate(kf.split(np.arange(N)), start=1):
        ...
        if variant != "film_baseline":
            _model_kwargs["bidirectional"] = bidir
        # -- architectural overrides (authentic: model arch only, not training) --
        if args.d_model and variant != "film_baseline":
            _model_kwargs["s4_d_model"] = args.d_model
        if args.d_state and variant != "film_baseline":
            _model_kwargs["s4_d_state"] = args.d_state
    ...
    lines = [
        ...
        f"  d_model={args.d_model or 'default'}  d_state={args.d_state or 'default'}",
        f"{'='*65}",
