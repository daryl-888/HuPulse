...
class Inception1d(nn.Module):
    ...
    def __init__(self, num_classes=1, input_channels=1, kss=[39,19,9], depth=6, bottleneck_size=32, nb_filters=32, use_residual=True,lin_ftrs_head=None, ps_head=0.5, bn_head=True, act_head="relu", concat_pooling=True):
        super().__init__()
...
class MInception1d(nn.Module):
    def __init__(self,
        ...
        # ??????
        dyn_dim = (len(kss) + 1) * nb_filters
        self.fusion_fc = nn.Sequential(
            nn.Linear(dyn_dim + static_hidden, fusion_hidden),
            nn.ReLU(inplace=True),
    ...
    def forward(self, ppg, static_feat):
        # ppg: (batch, 1, seq_len)
        x = self.backbone(ppg)       # -> (batch, dyn_dim, L')
        x = self.pool(x)             # -> (batch, dyn_dim, 1)
        x = torch.flatten(x, 1)      # -> (batch, dyn_dim)

        s = self.static_mlp(static_feat)  # -> (batch, static_hidden)
        fusion = torch.cat([x, s], dim=1)
        out = self.fusion_fc(fusion)      # -> (batch, num_BP)
        return out.squeeze()
