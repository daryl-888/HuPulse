...
class Model_Trainer:
    ...
    def Train_Model(self):
        ...
        for Epoch in range(Start_Epoch, Start_Epoch+self.Num_Epoch):
            try:
                ...
                for name, Test in zip(Test_Names, Test_List):
                    ...
                    #new
                    cur_best = best_test_metrics[Test_Name]
                    if Epoch_Test_MAE < cur_best['best_mae']:
                        cur_best['best_mae'] = float(Epoch_Test_MAE)
                        cur_best['best_epoch'] = Epoch
                        cur_best['best_sd'] = float(Epoch_Test_SD)
                        cur_best['best_r2'] = float(Epoch_Test_R2)
        ...
        #new    
        print('========== Best Test Metrics over All Epochs ==========')
        for name, m in best_test_metrics.items():
            print(f"{name}: best MAE = {m['best_mae']:.4f} "
                  f"at epoch {m['best_epoch']} "
                  f"(SD = {m['best_sd']:.4f}, R2 = {m['best_r2']:.4f})")
