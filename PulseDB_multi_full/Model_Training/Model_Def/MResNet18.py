...
# --- Mid-level Fusion ---
class MResnet18_1D(nn.Module):
    def __init__(self, num_static_features=3, num_BP=1, static_hidden=16, fusion_hidden=32):
        super(MResnet18_1D, self).__init__()
        # 1D???????
        self.resnet1d = Resnet18_1D()
        # ????MLP
        ...
        self.fusion_fc = nn.Sequential(
            nn.Linear(512 + static_hidden, fusion_hidden),
            nn.ReLU(inplace=True),
    ...
    def forward(self, ppg, static_feat):
        ...
        #print(f'ppg_feat:{ppg_feat.shape}, static_feat:{static_feat.shape}')
