...
MODEL_NAME = "ResNet50"  # ??: "ResNet18","ResNet50","Transformer","Inception","S4","LeNet"

OUT_DIR = "./bootstrap_cv_results_resnet50_10fold"
...
def get_model_instance(name: str, num_static_features=3, num_BP=1):
    ...
    if n in ["lenet","mlenet","lenet1d","m-lenet"]:
        return MLeNet1d(num_static_features=num_static_features, num_BP=num_BP)
    if n in ["s4","ms4","s4_1d"]:
        return MS4_1D(num_static_features=num_static_features, num_BP=num_BP)
    if n in ["inception","minception","inception1d"]:
        return MInception1d(num_static_features=num_static_features, num_BP=num_BP)
    if n in ["resnet18","mresnet18","resnet18_1d"]:
        return MResnet18_1D(num_static_features=num_static_features, num_BP=num_BP)
    if n in ["resnet50","mresnet50","resnet50_1d"]:
        return MResnet50_1D(num_static_features=num_static_features, num_BP=num_BP)
