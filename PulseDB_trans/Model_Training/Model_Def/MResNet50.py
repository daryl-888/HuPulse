...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, stride: int = 1, downsample=None, norm_layer=nn.BatchNorm1d):
        ...
        self.conv2 = conv3x1(width, width, stride)
...
class ResNet(nn.Module):
    def __init__(self, block, layers, zero_init_residual=False, norm_layer=nn.BatchNorm1d):
        ...
        # self.fc = nn.Linear(512 * block.expansion, num_BP)  # ??:fc??????????
...
# --- Mid-level Fusion ---
class MResnet50_1D(nn.Module):
    def __init__(self, num_static_features=3, num_BP=1, static_hidden=16, fusion_hidden=32):
        super(MResnet50_1D, self).__init__()
        # 1D???????
        self.resnet1d = Resnet50_1D()
        # ????MLP
        ...
        self.fusion_fc = nn.Sequential(
            nn.Linear(512 * Bottleneck.expansion + static_hidden, fusion_hidden),
            nn.ReLU(inplace=True),
    ...
    def forward(self, ppg, static_feat):
        # ppg: (batch, 1, seq_len)
        ppg_feat = self.resnet1d(ppg)  # (batch, 2048)
        static_feat = self.static_mlp(static_feat)  # (batch, static_hidden)
        fusion = torch.cat([ppg_feat, static_feat], dim=1)
