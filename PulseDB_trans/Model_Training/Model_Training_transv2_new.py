...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return (self.Input[idx, :].astype(np.float32), static_feat), self.Label[idx], self.IsNormal[idx]  # x, y
...
def Build_Dataset(Path, Label, show_info=True, augment=False):  # 新增 augment 参数
    ...
    # --- 新增：读取 Is_Normal (0/1 二值) ---
    is_normal = np.array(Data['Subset']['Is_Normal'], dtype=np.float32).flatten()
    # ------------------------------------
    ...
    for i in range(N):
        ...
        features = [age[i], bmi[i], gender[i], is_normal[i]]
        if (
            np.any(np.isnan(features)) or np.any(np.isinf(features)) or
            np.any(np.isnan(signals[i])) or np.any(np.isinf(signals[i]))
        ):
    ...
    bmi = bmi[mask]
    gender = gender[mask]
    is_normal = is_normal[mask] # 过滤 is_normal
    label = label[mask]
    after_num = len(age)
    ...
    # 将 is_normal 传给 Dataset 类
    # 注意：你需要确保你的 Dataset 类的 __init__ 方法能够接收这个新参数
    return Dataset(signals, age, bmi, gender, is_normal, label, augment=augment, noise_std=0.1)
